...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: before } = await supabase.from("accounts").select(fields.join(",")).eq("id", id).single();
  ...
  if (changed.length) {
    await supabase.from("audit_logs").insert(changed.map((f) => ({
      actor_id: user.id, action: "update", entity: "account", entity_id: id,
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const auth = await getUserOr401();
  if ("error" in auth) return auth.error;
  const { error } = await auth.supabase.from("accounts").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
